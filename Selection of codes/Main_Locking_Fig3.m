...
%% Calculates locking and ensembles

N_sh=10; %In the paper we used 1000 shuffled iterations
clus=10;
count=0;
prctile_th=99;
...
%     phase_di=discretize(phase_down,-pi:2*pi/disc_phase:pi);    
    for i=1:length(phase_down)
        if i*new_bin>size(spikes_d,2)
            spk_do(:,i)=sum(spikes_d(:,(i-1)*new_bin+1:end),2);
        else
...
%% Figures fraction of cells locked to the sequences
% Figre 3b
L=0;
NL=0;
for w=1:15
...
%% Distance within and across ensembles - Extended data Fig 10 k
...
%% Continuity of mean phase - Extended data Fig 7e,f
