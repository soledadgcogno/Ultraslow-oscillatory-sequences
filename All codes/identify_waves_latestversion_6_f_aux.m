...
if make_fig==1
    ...
    %       fig=figure;
    set(gcf, 'Units', 'Normalized', 'OuterPosition', [0 0 1 0.28]);
    hold on
    for i=1:size(spikes,1)
        %                     scatter((1:size(spikes_d,2))./8,i*spikes_d(sorting_descend(i),:),5,'k','filled')
        scatter((1:size(spikes,2))./8,i*spikes(sorting_FRp(i),:),5,'k','filled')
        alpha 0.2
    end
    axis([-inf inf 1 inf]);
    ...
    subplot(2,1,2)
    set(gcf, 'Units', 'Normalized', 'OuterPosition', [0 0 1 0.28]);
    plot((1:size(spikes,2))./8,phase,'k','linewidth',1.5);
    hold on
    for i=1:size(table_u,1)
%         plot((table_u(i,1):table_u(i,2))./8,phase(table_u(i,1):table_u(i,2)),'linewidth',2,'color',cc(i,:));
        plot((table_u(i,1):table_u(i,2))./8,phase(table_u(i,1):table_u(i,2)),'linewidth',2,'color','c');
