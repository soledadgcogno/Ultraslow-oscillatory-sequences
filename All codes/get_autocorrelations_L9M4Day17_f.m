...
load([rec_data_path ['WS_Osc_14_','L09M4','.mat']]);
% dt
dt=WS_stat.dt(day,s);
if isinteger(dt)
else
...
figure
[h,edges]=histcounts(peak_freq,[0:0.002:0.1]);
binsi=edges(2)-edges(1);
bar(edges(1:end-1)+(binsi/2),h);
xlabel('Frequency (Hz)');
ylabel('Counts');
axis([0 0.045 0 inf]);
box off
...
b=discretize(period_ps,[10,50,70,90,120,160,250,400]);
cc=hsv(7);
file_name_anat=[dpath ['Anat_',mouse,'_day',num2str(day),'_MUnit',num2str(munit),'.mat']];
load(file_name_anat,'-mat');
figure
...
%% Examples of individual cells - MVL
...
figure
subplot(2,1,1)
plot((val(cell_high_locking,:)),'k','linewidth',2.5)
xticks([1 maxlag maxlag*2])
xticklabels({-maxlag/2 0 maxlag/2});
xlabel('Time (s)');
set(gca,'fontsize',20);
...
box off
subplot(2,1,2)
plot(Powerfreq2(1:40),(pwelch_fft(cell_high_locking,1:40)),'k','linewidth',2.5);
hold on
% plot(f(1:70),fft_(cell_high_locking,1:70),'k','linewidth',2.5);
% title(MVL(1))
ylabel('PSD (mV^2/Hz)');
...
figure
subplot(2,1,1)
plot(val(cell_med_locking,:),'k','linewidth',2)
xticks([1 maxlag maxlag*2])
xlabel('Time (s)');
set(gca,'fontsize',20);
h = gca; h.YAxis.Visible = 'off';
...
box off
subplot(2,1,2)
plot(Powerfreq2(1:40),pwelch_fft(cell_med_locking,1:40),'k','linewidth',2);
% plot(f(1:70),fft_(cell_med_locking,1:70),'k','linewidth',2.5);
% title(MVL(285))
ylabel('PSD (dB/Hz)');
xlabel('Frequency (Hz)');
set(gca,'fontsize',20);
...
figure
subplot(2,1,1)
plot(val(cell_med_locking_2,:),'k','linewidth',2)
xticks([1 maxlag maxlag*2])
xlabel('Time (s)');
set(gca,'fontsize',20);
h = gca; h.YAxis.Visible = 'off';
...
box off
subplot(2,1,2)
plot(Powerfreq2(1:40),pwelch_fft(cell_med_locking_2,1:40),'k','linewidth',2);
% plot(f(1:70),fft_(cell_med_locking,1:70),'k','linewidth',2.5);
% title(MVL(285))
ylabel('PSD (dB/Hz)');
xlabel('Frequency (Hz)');
...
figure
subplot(2,1,1)
plot(val(cell_low_locking,:),'k','linewidth',2.5)
xticks([1 maxlag maxlag*2])
xticklabels({-maxlag/2 0 maxlag/2});
xlabel('Time (s)');
set(gca,'fontsize',20);
...
axis square
subplot(2,1,2)
plot(Powerfreq2(1:40),pwelch_fft(cell_low_locking,1:40),'k','linewidth',2.5);
% plot(f(1:70),fft_(cell_low_locking,1:70),'k','linewidth',2.5);
% title(MVL(147))
ylabel('PSD (dB/Hz)');
xlabel('Frequency (Hz)');
set(gca,'fontsize',20);
...
figure
subplot(2,1,1)
plot(val(cell_low_locking_not_locked,:),'k','linewidth',2)
xticks([1 maxlag maxlag*2])
xticklabels({-maxlag/2 0 maxlag/2});
ylabel('Autocorrelation');
xlabel('Time (s)');
...
axis square
subplot(2,1,2)
plot(Powerfreq2(1:40),pwelch_fft(cell_low_locking_not_locked,1:40),'k','linewidth',2.5);
% plot(f(1:70),fft_(cell_low_locking_not_locked,1:70),'k','linewidth',2.5);
% title(MVL(10))
box off
h=xline(freq_wave,'--r','linewidth',2.5);
% h=xline(freq_wave/2,'--b','linewidth',2.5);
...
figure
subplot(2,1,1)
plot(val(cell_low_locking_not_locked2,:),'k','linewidth',2)
xticks([1 maxlag maxlag*2])
xticklabels({-maxlag/2 0 maxlag/2});
ylabel('Autocorrelation');
xlabel('Time (s)');
...
axis square
subplot(2,1,2)
plot(Powerfreq2(1:40),pwelch_fft(cell_low_locking_not_locked2,1:40),'k','linewidth',2.5);
% plot(f(1:70),fft_(cell_low_locking_not_locked,1:70),'k','linewidth',2.5);
% title(MVL(10))
box off
h=xline(freq_wave,'--r','linewidth',2.5);
...
% for i=1:N
%     aux=cumsum(pwelch_fft(i,:))./sum(pwelch_fft(i,:));
%     thr(i)=find(aux>0.9,1);
% end

b=discretize(peak_fft,[0.0017,3.6*0.0018,4.1*0.0018,5.1*0.0018,7*0.0018,10]);
cc=copper(5);
file_name_anat=[dpath ['Anat_',mouse,'_day',num2str(day),'_MUnit',num2str(munit),'.mat']];
load(file_name_anat,'-mat');
figure
...
for i=1:N
    ...
    y=Anat.pos{1,i}(:,2);
    ov=Anat.overlap{1,i};
    scatter(x(find(ov<2)),y(find(ov<2)),1,cc(b(i),:),'filled');
end
box on
xlabel('X [\mum]');
ylabel('Y [\mum]');
...
b=discretize(thr,[0,50,100,150,200,600]);
cc=inferno(5);
file_name_anat=[dpath ['Anat_',mouse,'_day',num2str(day),'_MUnit',num2str(munit),'.mat']];
load(file_name_anat,'-mat');
figure
