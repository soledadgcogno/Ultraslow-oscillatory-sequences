...
%     spikes_d=spikes_d;
    ...
    clear Ens
    [~,sorting_w,~]=get_sorting(spikes_d);
...
save('C:\Users\xscogno\MATLAB\Flavio2\Waves\Semi final scripts\Final scripts\Outputs\locking_ensemble_Pearson_PaS\locking_ensembles_Pearson_pas.mat','session')
...
%For one session ; session=14
in_session14=find(index_count==14);

H=hist3([inf_ens(in_session14)',real_ens(in_session14)'],'Edges',{1:1:10 1:1:10});
figure
imagesc(H);
colormap magma
xticks([1 5 10])
...
H=hist3([inf_ens_sh(in_session14)',real_ens(in_session14)'],'Edges',{1:1:10 1:1:10});
figure
imagesc(H);
colormap magma
xticks([1 5 10])
