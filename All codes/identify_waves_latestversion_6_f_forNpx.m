function [table_u_r,spk_trace_aux,spk_trace_aux_full,sorting,cells_to_keep]=identify_waves_latestversion_6_f_forNpx(num_clus_discr,bin_size,dt_s,make_fig,spikes_data,chunk_ini,chunk_end)

sf=1/bin_size; 
min_events=50;
dt_s=floor(dt_s*sf); %number of bins used to bin the data
%Generates sorting
sorting_ini=chunk_ini*floor(1/bin_size);
sorting_fin=chunk_end*floor(1/bin_size);
clear spk_trace_aux spk_trace_aux_full; spk_trace_aux=spikes_data(:,floor(sorting_ini):floor(sorting_fin));
clear aux; aux=sum(spk_trace_aux,2); spk_trace_aux(aux<min_events,:)=[]; spk_trace_aux_full=spikes_data; spk_trace_aux_full(aux<min_events,:)=[];
all_cells=1:size(spikes_data,1);
cells_to_keep=setdiff(all_cells,find(aux<min_events));
[~,sorting]=get_sorting_npx(spk_trace_aux);
spikes=spk_trace_aux_full;
% b=flip(b);
% make_rasterplot(spk_trace_aux_full(:,starting_point_plot:end),b,bin_size,0);

%Calculation of phase of the oscilation
N=size(spk_trace_aux,1);
    %for spk_chunk
[coeff,score,~] = pca(spk_trace_aux');
phase_f_chunk=(atan2(smooth(score(:,2),floor(1*dt_s)),smooth(score(:,1),floor(1*dt_s))));
PC1_chunk=spk_trace_aux(:,1:end)'*coeff(:,1);
PC2_chunk=spk_trace_aux(:,1:end)'*coeff(:,2);
phase_f_chunk2=(atan2(smooth(PC2_chunk,floor(1*dt_s)),smooth(PC1_chunk,floor(1*dt_s))));
    %for the entire matrix
PC1=spk_trace_aux_full(:,1:end)'*coeff(:,1);
PC2=spk_trace_aux_full(:,1:end)'*coeff(:,2);
phase_f=(atan2(smooth(PC2,floor(1*dt_s)),smooth(PC1,floor(1*dt_s))));
...
%Check whether waves go upwards or inwards in the selected chunk
aux=(diff(phase_f_chunk2));
