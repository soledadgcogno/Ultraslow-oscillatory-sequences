        ...
        for m=1:size(C,1) % Loop on all spatial bins
            if sum(idx==m)>0
                ...
                if length(aux2)>1
                    ...
                    %                 prctile_1(C(m,1),C(m,2),count_s)=prctile(circp_pos_sh(C(m,1),C(m,2),:),1);
                    %                 prctile_5(C(m,1),C(m,2),count_s)=prctile(circp_pos_sh(C(m,1),C(m,2),:),5);
                    %                 prctile_10(C(m,1),C(m,2),count_s)=prctile(circp_pos_sh(C(m,1),C(m,2),:),10);
...
figure
scatter(circp_pos(:),prctile_99(:),'k','filled');
ylabel({'99^t^h percentile' ; 'Shuffled data'});
set(gca,'fontsize',16);
xlabel('Experimental data');
% title([num2str(n_bins),'x',num2str(n_bins),' bins']);
title(['100\mum x 100\mum bins']);
hl=refline(1,0);
hl.LineWidth=5;
hl.LineStyle='--';
% figure
...
%Params for ttemporal bins
n_bins=30;

%Arrays initialization
circp_pos_sh=nan(n_bins,N_sh);
meanp_pos_sh=nan(n_bins,N_sh);
occupancy_mat=zeros(1,n_bins);
occupancy_mat_b=zeros(1,n_bins);
meanp_pos=nan(n_bins,1);
circp_pos=nan(n_bins,1);
prctile_1=nan(n_bins,1);
prctile_5=nan(n_bins,1);
prctile_10=nan(n_bins,1);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% Binning of cells' position
idx=discretize(sorting_descend,0:floor(N/n_bins):N);
idxo=idx;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% Calculation of local phases per sequence
count_s=0;
for s=19%17:19
    count_s=count_s+1;
    mean_p=mean_p_seq(:,s);
    clear aux; aux=~isnan(mean_p);
    mean_p=mean_p(aux);
    idx=idx(aux);
    Nn=length(mean_p);

    for sh=1:N_sh
        mean_p_sh=mean_p(randperm(Nn));
        for m=1:n_bins % Loop on all spatial bins
            clear aux aux2; aux=(mean_p_sh(idx==m)); aux2=aux(~isnan(aux));
            meanp_pos_sh(m,sh)=circ_mean(aux2);
            circp_pos_sh(m,sh)=circ_r(aux2);
        end
    end

    for m=1:n_bins % Loop on all spatial bins
        occupancy_mat(m,count_s)=sum(idx==m);
        clear aux aux2; aux=(mean_p(idx==m)); aux2=aux(~isnan(aux));
        occupancy_mat_b(m,count_s)=length(aux2);
        meanp_pos(m,count_s)=circ_mean(aux2);
        circp_pos(m,count_s)=circ_r(aux2);

        %             prctile_1(C(m,1),C(m,2),count_s)=prctile(circp_pos_sh(C(m,1),C(m,2),:),1);
        %             prctile_5(C(m,1),C(m,2),count_s)=prctile(circp_pos_sh(C(m,1),C(m,2),:),5);
        %             prctile_10(C(m,1),C(m,2),count_s)=prctile(circp_pos_sh(C(m,1),C(m,2),:),10);

        prctile_90(m,count_s)=prctile(circp_pos_sh(m,:),90);
        prctile_95(m,count_s)=prctile(circp_pos_sh(m,:),95);
        prctile_99(m,count_s)=prctile(circp_pos_sh(m,:),99);
    end

    %Reaccomodate matrix for occupancy and plot
    occupancy_mat_b_plot=occupancy_mat_b'; 
    figure
    heatmap(flip(occupancy_mat_b_plot));
    %     c=colorbar;
    set(gca,'fontsize',10);
    ylabel('Y bin #');
    xlabel('X bin #');
    ...
    cc=cool(n_bins);
    cc_idx=discretize(mean_p,-pi:2*pi/n_bins:pi);
    spk_mini=spikes_d(:,table_u(19,1):table_u(19,2));
    figure
    hold on
    for n=1:N
        if isnan(cc_idx(sorting_descend(n)))
%             scatter(1:size(spk_mini,2),n*spk_mini(sorting_descend(n),:),2,'color','r');
        else
            scatter(1:size(spk_mini,2),n*spk_mini(sorting_descend(n),:),2,cc(cc_idx(sorting_descend(n)),:));
        end
    end
    ...
    figure
    for m=1:n_bins % Loop on all spatial bins
        if isnan(circp_pos(m,count_s))
        else
            subplot(6,5,m)
            histogram(circp_pos_sh(m,:),[0:1/40:1]);
            title({['Bin = ',num2str(m)]});
            axis([0 1 -inf inf])
            hold on
            xline( prctile_99(m,count_s),'b-','linewidth',5);
            xline( circp_pos(m,count_s),'r-','linewidth',5);
            ylabel('Counts');
            xlabel('MVL');
            set(gca,'fontsize',16);
        end
