...
for day=1:days_num
    ...
    for s=1:ses_num
        ...
        if (length(num2str(dates.days(day)))==8)
            ...
            dpath=strcat('Z:\Flavio\2P\',mouse,'\',num2str(dates.days(day)),'\MUnit_',num2str(munit),'\suite2p\plane0\');
            
            file_name=strcat(dpath,'Fall.mat');
            file_name_snr= [snr_path strcat(mouse,'\Flavio_2P_',mouse,'_',num2str(dates.days(day)),'_Munit_',num2str(munit),'.csv')];
            
            %                                     file_name_snr= [snr_path strcat('Flavio#2P#',mice(m,1:4),'#',num2str(dates.days(day)),'#',num2str(ses),'#F_',mice(m,1:4)...
            %                             ,'_',num2str(dates.days(day)),'_plane1_proc.mat.csv')];
        else
            dpath=strcat('Z:\Flavio\2P\',mouse,'\0',num2str(dates.days(day)),'\MUnit_',num2str(munit),'\suite2p\plane0\');
...
%                     save([spath,'spikes_120ms_Do_THR',num2str(res),'_SNRH_FoV1','_',mouse,'_day',num2str(day),'_MUnit',num2str(munit)],'spikes_d_s','cells_d');
                      save([spath,'spikes_120ms_Do_THR1p5_SNRH_FoV1','_',mouse,'_day',num2str(day),'_MUnit',num2str(munit)],'spikes_d_s','cells_d');
